...
class WarpRegistrationBot:
    ...
    async def register_account(self, temp_mail_client: TempMailAPIClient) -> Optional[str]:
        ...
        # 尝试注册（使用代理或直连）
        # 无论是否使用代理，都允许重试
        max_attempts = config.MAX_PROXY_RETRIES
        for attempt in range(max_attempts):
            ...
            if proxy_str:
                ...
                logger.info(f"[{email}] 第{attempt + 1}次尝试，使用代理: {proxy_str}")
            else:
                logger.info(f"[{email}] 第{attempt + 1}次尝试，使用直连（无代理）")

            # 初始化httpx异步客户端
            self.async_client = httpx.AsyncClient(
                proxy=proxy,  # proxy 为 None 时表示直连
                verify=False,
        ...
        logger.error(f"[{email}] 尝试了{max_attempts}次后仍然失败")
        return None
    ...
    async def registration_worker(self, worker_id: int):
        ...
        while self.running:
            try:
                ...
                logger.info(f"[Worker-{worker_id}] 当前账户数: {current_count}/{self.target_accounts}")
                
                if current_count >= self.target_accounts:
                    logger.info(f"[Worker-{worker_id}] 已达到目标账户数，等待30秒后继续检查...")
                    await asyncio.sleep(30)
                    continue

                logger.info(f"[Worker-{worker_id}] 开始新的注册尝试...")
                ...
                if local_id:
                    ...
                    logger.info(f"[Worker-{worker_id}] ✅ 注册成功: {email}，等待5秒后继续...")
                    await asyncio.sleep(5)
                    logger.info(f"[Worker-{worker_id}] 准备进行下一次注册...")
                else:
                    async with self.stats_lock:
                        self.stats["failed"] += 1
                    logger.error(f"[Worker-{worker_id}] ❌ 注册失败: {email}，等待30秒后重试...")
                    await asyncio.sleep(30)
                    logger.info(f"[Worker-{worker_id}] 准备重试注册...")

            except Exception as e:
                logger.error(f"[Worker-{worker_id}] 工作线程异常: {e}", exc_info=True)
                async with self.stats_lock:
                    self.stats["failed"] += 1
                logger.info(f"[Worker-{worker_id}] 等待10秒后继续...")
